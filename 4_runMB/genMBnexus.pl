...
my @fastaFiles = glob("../3_simulate/*.fasta");
foreach my $fasta (@fastaFiles)
{
    if ($fasta =~ m/\.\.\/3_simulate\/(\S+)\.fasta/)
    {
	my $prefix = $1;
	my %seqs = ();
	my $tax = "";
	...
	print OUT1 "mcmcp nruns=4 nchains=4 ngen=1111112 samplefreq=100 burninfrac=.1 printfreq=10000 savebrlens=YES starttree=random mcmcdiagn=YES diagnfreq=10000;\n";
